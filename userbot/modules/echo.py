...
async def echo(event):
    ...
    vegetaevent = await edit_or_reply(event, "`Menambahkan Echo ke pengguna...`")
    user, rank = await get_user_from_event(event, vegetaevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(vegetaevent, f"**Error :**\n`{str(e)}`")
    else:
        await edit_or_reply(vegetaevent, "**Sukses**")
