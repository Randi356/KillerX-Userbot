...
async def autobot():
    ...
    await bot.send_message(
        BOTLOG_CHATID, "➕ **LAGI GUA BIKIN ASISTAN BOT DI @BOTFATHER TUNGGU AJA DULU,**"
    )
    who = await bot.get_me()
    name = who.first_name + " Assistant Vegeta-Userbot"
    if who.username:
        username = who.username + "_userbot"
    else:
        username = "userbot" + (str(who.id))[5:] + "userbot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ...
        await bot.send_message(bf, f"@{username}")
        await asyncio.sleep(1)
        await bot.send_file(bf, "userbot/utils/styles/vegeta.jpg")
        await asyncio.sleep(3)
        await bot.send_message(bf, "/setabouttext")
        await asyncio.sleep(1)
        await bot.send_message(bf, f"@{username}")
        await asyncio.sleep(1)
        await bot.send_message(bf, f"SUCCESFULL ASSISTANT DONE {who.first_name}")
        await asyncio.sleep(3)
        await bot.send_message(bf, "/setdescription")
        await asyncio.sleep(1)
        await bot.send_message(bf, f"@{username}")
